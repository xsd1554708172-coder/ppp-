...
def main():
    ...
    xlsx = os.path.join(cfg["output_dir"], "PPP_第9部分第3批B_无泄漏版_项目级风险识别增强模型比较.xlsx")
    ...
    pred_out.to_csv(os.path.join(cfg["output_dir"], "PPP_project_risk_best_model_predictions_3B_no_leak.csv"), index=False, encoding="utf-8-sig")
    save_roc_plot(roc_points, "项目级风险识别模型 ROC 曲线比较（3批B，无泄漏版）", os.path.join(cfg["output_dir"], "PPP_第9部分第3批B_无泄漏版_ROC曲线比较.png"))
    save_metric_plot(results_df, "项目级风险识别模型性能比较（3批B，无泄漏版）", os.path.join(cfg["output_dir"], "PPP_第9部分第3批B_无泄漏版_模型性能比较.png"))
    save_importance_plot(imp_tables[best], f"最佳模型特征重要性（3批B，无泄漏版）：{best}", os.path.join(cfg["output_dir"], "PPP_第9部分第3批B_无泄漏版_最佳模型特征重要性.png"))
